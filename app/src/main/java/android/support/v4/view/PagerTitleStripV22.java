 ...
 * {@link ViewPager}. It is intended to be used as a child view of a ViewPager widget in your XML
 * layout. Add it as a child of a ViewPager in your layout file and set its android:layout_gravity
 * to TOP or BOTTOM to pin it to the top or bottom of the ViewPager. The title from each page is
 * supplied by the method {@link android.support.v4.view.PagerAdapter#getPageTitle(int)} in the
 * adapter supplied to the ViewPager. <p/> <p>For an interactive indicator, see {@link
 * android.support.v4.view.PagerTabStrip}.</p>
 */
...
public class PagerTitleStripV22 extends ViewGroup implements ViewPager.Decor {
    ...
    /**
     * Set the color value used as the base color for all displayed page titles. Alpha will be
     * ignored for non-primary page titles. See {@link #setNonPrimaryAlpha(float)}.
     *
     ...
     * dimension units. <p/> <p>Example: to set the text size to 14px, use
    ...
    /**
     * Set the {@link Gravity} used to position text within the title strip. Only the vertical
     * gravity component is used.
     *
